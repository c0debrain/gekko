...
method.init = function() {
    this.weightFileName = "weights/lookbackPercept-ethtrx-direct-lab.json";
...
method.check = function(candle) {
    ...
    var predictDirection = this.network.activate(this.lookbackCheckInput);
    ...
    //log.info(this.lookbackCheckInput);
    log.info("Predict: "+predictDirection);

    if(predictDirection > 0.9 && !this.open_order)
    ...
    //if(predicted_value > .8 && !this.open_order)
    {
        ...
        log.info("Buy: $"+candle.close+" predictDirection: "+predictDirection);
        //log.info(this.lookbackCheckInput);
    ...
    //} else if(this.open_order && predicted_value < .5){
        ...
        log.info("Sold: $"+candle.close+" predictDirection: "+predictDirection+" profit%: "+profitPercent);
...
method.getLookbackInputNormalized = function(lookbackData) {
    var lookbackInput = [];
    for(var i=0;i<lookbackData.length;i++) {
