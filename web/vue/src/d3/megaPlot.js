...
export default function(_data, _trades, _height) {
  ...
  var margin = {top: 20, right: 20, bottom: 10, left: 50},
            width = window.innerWidth + 60 - margin.left - margin.right,
            height = _height - margin.top - margin.bottom;

  var dim = {
        width: width, height: height,
        margin: { top: 20, right: 50, bottom: 25, left: 60 },
        ohlc: { height: 300 },
        indicator: { height: 75, padding: 5 }
    };
    dim.plot = {
    ...
    var tradearrow = techan.plot.tradearrow()
            ...
            .y(function(d) {                // Display the buy and sell arrows a bit above and below the price, so the price is still visible
                if(d.type === 'buy') return y(d.price)+1;
                if(d.type === 'sell') return y(d.price)-1;
